...
const LEONEX_GULPFILE_VERSION = '1.9.0';    // must match the version in package.json!
            ...
            if( ['watch', 'copy'].indexOf(TASK) > -1 ) {
                ...
                if( copyInstructions.length > 0 || groupCopyInstructionsCount > 0 || taskMatrix[frontend]['afterBuild'].copy.length > 0 ) {
                    ...
                    if( TASK == 'watch' ) {
                        ...
                        if( reloadDeps.length == 0 ) {
                            reloadDeps = [taskNameCopy];
                        }
...
function buildFrontendGlobs(frontendConfig) {
    ...
    globs.plain.src = frontendConfig.plain_files.map(function(relativePath) {
        return srcDir + '/' + relativePath;
    });
    globs.plain.watch = globs.plain.src;
...
var buildTasks = {
    ...
    css: function(config) {
        return function() {
            ...
            switch( config.css_preprocessor ) {
                case 'sass':
                    handler = lazypipe().pipe(function() {
                        return $.sass({
                            precision: 8
                ...
                case 'less':
                    handler = lazypipe().pipe(function() {
                        return $.less()/*.on('error', console.error.bind(console))*/;
                    });
                    break;
            }
            ...
            return gulp.src(config.globs.css.src)
                ...
                .pipe($.postcss([
                    autoprefixer,
                    postcssInlineSvg({path: base + config.folders.sources.images}),
                    postcssSvgo,
                    postcssCsso
                ]))
    ...
    plain: function(config) {
        return function() {
            return gulp.src(config.globs.plain.src, {base: config.globs.plain.base})
                .pipe(gulp.dest(config.assembleDest(config.globs.plain.dest)));
        };
    },
